...
// Original = https://wordpress.org/plugins/bbpress-post-topics/
...
class BBP_PostTopics {
	...
	function build_new_topic( $post, $topic_forum ) {
		...
		if( isset( $strings['topic-text'] ) ) {
			...
			$topic_content = '%excerpt<br /><br />' . sprintf( __('[A teljes tartalom itt olvasható: <a href="%s">%s</a>]','bbpress-post-topics'), '%url', '%url' );
		...
		$shortcodes = array(
			...
			'%excerpt'	=> ( empty( $post->post_excerpt ) ? bbppt_post_discussion_get_the_content($post->post_title, 1500) : apply_filters('the_excerpt', $post->post_excerpt) ),
...
function bbppt_post_discussion_get_the_content( $content, $cut = 0 ) {
	...
	return 'Kérdésed, vagy véleményed van a(z) <strong>'.$content.'</strong> című tartalommal kapcsolatban?';
}
